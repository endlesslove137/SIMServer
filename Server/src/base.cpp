...
void localtime( struct tm &pTm, time64 &pTime )
{
#ifdef LINUX
...
//    return atomic_cas(value, new_value, old_value);
...
WD_API int semaphore_get(semaphore_t* semaphore, uint32 milli_secs)
{
    ...
    milli_secs += tv.tv_sec*1000+tv.tv_usec/1000;

    timespec ts;
    ts.tv_sec = tv.tv_sec + milli_secs/1000;
    ts.tv_nsec = milli_secs%1000*1000000;
